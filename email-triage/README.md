...
- Work always skips Inbox and must remain unread. For other categories, mark a message read by removing Gmail’s `UNREAD` label only when it is not in Inbox, does not have `Action`, and is not `Urgency/high`. Otherwise preserve its read state.
...
- GitHub, Spam, and Potential Spam have no urgency regardless of Jev’s result. For other categories, apply `Urgency/high` or `Urgency/low` only when Jev chooses it with probability at least 0.70 and a top-two gap of at least 0.15; otherwise apply no urgency label without adding `Review`.
